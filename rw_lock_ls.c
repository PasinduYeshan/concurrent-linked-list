...
void *threadFn(void *arg) {
    ...
    for (int i = threadId; i < m; i += numThreads) {
        ...
        if (operation_array[i] == 0) { // Member operation 
            pthread_rwlock_rdlock(&mutex);
            Member(value, *head_pp);
            pthread_rwlock_unlock(&mutex);
        } else if (operation_array[i] == 1) { // Insert operation
...
// Run linked list operations with read-write locks
double RunLinkedListOperations(int n, int m, float prob_member, float prob_insert, float prob_delete, int numThreads) {
    ...
    pthread_rwlock_init(&mutex, NULL); // Initialize mutex
    ...
    for (int t = 0; t < numThreads; t++) {
        ...
        pthread_create(&threads[t], NULL, threadFn, (void *) args);
    }
...
int main() {
    ...
    for (int i = 0; i < testRuns; i++) {
        results[i] = RunLinkedListOperations(n, m, prob_member, prob_insert, prob_delete, numThreads);
    }
